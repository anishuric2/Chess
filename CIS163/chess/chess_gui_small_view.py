...
from chess_model import ChessModel, MoveValidity, UndoException, ChessAi
...
class GUI:
    ...
    def __init__(self) -> None:
        ...
        self.chess_ai = ChessAi(self.__model)
    ...
    def run_game(self) -> None:
        ...
        while running:
            for event in pg.event.get():
                if self.__model.current_player == Player.BLACK:
                    self.chess_ai.make_move()
                ...
                if event.type == pg.MOUSEBUTTONDOWN:
                    ...
                    if not self._piece_selected and piece:
                        ...
                        mv = Move(
                            self._first_selected[0], self._first_selected[1], y, x
                        )
                        ...
                        if incheck:
                            ...
                            if complete:
                                self._side_box.append_html_text(
                                    f"{player_color} is in CHECKMATE!<br />GAME OVER!"
                                )
                            else:
                                self._side_box.append_html_text(
                                    f"{player_color} is in CHECK!<br />"
                                )

                        self._piece_selected = False
                    else:
                        self._piece_selected = False
                    # if self.__model.current_player == self.chess_ai.model.player:
                    #     self.chess_ai.make_move()
            ...
            for y in range(0, 8):
                ...
                if draw_piece is not None:
                    if draw_piece.player == Player.BLACK:
                        ...
                        d = GUI.white_sprites
                    self._screen.blit(
                        copy.deepcopy(d[draw_piece.type()]),
